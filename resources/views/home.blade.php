...
@section('content')
    ...
    <main class="container-fluid">
        ...
        @endif
        @isset($letypecamera)
            <h2 class="text-title mb-3">{{ $letypecamera->name }}</h2>
        @endif
        @isset($user)
        ...
        <div class="card-columns">
            @foreach($images as $image)
                <div class="card @if($image->adult) border-danger @endif" id="image{{ $image->id }}">
                    ...
                    @isset($image->typecamera->name)
                        <hr/>
                        <p class="card-text">Type de Camera: {{ $image->typecamera->name }}</p>
                    @endisset
                    </div>
                            ...
                            <span class="pull-right">
                                @adminOrOwner($image->user_id)
                                    ...
                                    <span class="menuIcons" style="display: none">
                                        ...
                                        <a class="typecamera-edit"
                                           data-id="{{ $image->typecamera_id }}"
                                           href="{{ route('image.update', $image->id) }}"
                                           data-toggle="tooltip"
                                           title="@lang('Changer le type de caméra')">
                                           <i class="fa fa-edit"></i>
                                        </a>
                                        <a class="adult-edit"
    ...
    <div class="modal fade" id="changeTypeCamera" tabindex="-1" role="dialog" aria-labelledby="typeCameraLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="typeCameraLabel">@lang('Changement du type de caméra')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('script')
    ...
    <script>
        $(() => {
            ...
            $('a.typecamera-edit').click((e) => {
                e.preventDefault()
                let that = $(e.currentTarget)
                $('select').val(that.attr('data-id'))
                $('#editForm').attr('action', that.attr('href'))
                $('#changeTypeCamera').modal('show')
            })
