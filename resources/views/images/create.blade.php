...
@section('card')
    ...
    @component('components.card')
        ...
        <form method="POST" action="{{ route('image.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="typecamera_id">@lang('Type de Caméra')</label>
                <select id="typecamera_id" name="typecamera_id" class="form-control">
                    @foreach($typecamera as $typeCameras)
                        <option value="{{ $typeCameras->id }}">{{ $typeCameras->name }}</option>
                    @endforeach
                </select>
            </div>
