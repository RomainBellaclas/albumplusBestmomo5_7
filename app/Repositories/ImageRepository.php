...
class ImageRepository
{
    ...
    public function store($request)
    {
        ...
        $image->typecamera_id = $request->typecamera_id;
        $image->adult = isset($request->adult);
        $image->name = $path;
        $request->user()->images()->save($image);
        $request->typecamera()->images()->save($image);
    }
    ...
    public function getImagesForTypeCamera($slug)
    {
        $query = Image::latestWithUser ()->whereHas ('typecamera', function ($query) use ($slug) {
            $query->whereSlug ($slug);
        });
